...
#endif
    //dprintf print through BufferedSerial *ser;
    dprintf("Transmitter Application, FeedTheReef" );
...
int Send(char *text){
	    int t = strlen(text);
	    if(transmitter_booted==false){
	    	Transmitter();
	    }else{		    	
		    if(t<BUFFER_SIZE){	
	    		Radio->Send(text,t);
	    	}else{
	    		dprintf("Message too long: %d", t);    	
	    		}    	
        }
        return 1;
}
